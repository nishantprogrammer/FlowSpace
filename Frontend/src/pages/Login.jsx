...
export default function Login() {
  ...
  const handleDemo = async () => {
    try {
      await login('admin@flowspace.dev', 'Demo1234!');
      navigate('/dashboard');
    } catch (e) {
      setError('Demo login failed. Did you seed the DB?');
    }
  };
  ...
  return (
    <div className="flex min-h-screen w-full bg-bg text-text-primary">
      <div className="hidden lg:flex w-[60%] flex-col justify-center p-24" style={{ backgroundColor: '#0A0A0A', color: '#F0EEE8' }}>
        <h1 className="font-serif text-[52px] leading-tight mb-4">Sign in to Flowspace</h1>
        ...
        <p className="max-w-md text-[#F0EEE8]/70 text-lg leading-relaxed">
          A team task manager designed to look handcrafted and editorial. No distractions, just content and space.
        </p>
      </div>

      <div className="flex-1 flex flex-col justify-center px-12 sm:px-24 bg-surface border-l border-border">
        <div className="w-full max-w-sm mx-auto">
          <h2 className="text-2xl font-serif mb-8 lg:hidden">Sign in</h2>
          ...
          <div className="mt-8 flex flex-col space-y-4 text-center">
            <button onClick={handleDemo} className="text-sm text-text-muted hover:text-text-primary transition-colors">
              Use demo account
            </button>
            <div className="text-sm text-text-muted">
              Don't have an account? <Link to="/signup" className="text-text-primary border-b border-text-primary pb-0.5">Sign up</Link>
            </div>
          </div>
